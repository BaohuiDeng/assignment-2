...
app.get('/populatedb', (req, res) => {
	...
	console.log("Create endpoint called populatedb:");
	 var db = client.db(database); /* connect to assignment-2 db */
	 var col = db.collection(collection1);
	 col.insert({matriculationNumber:sample_students[0],firstName:sample_students[1],
		LastName:sample_students[2],Age:sample_students[3],StudyProgram:sample_students[4],
		AttendingCourses:sample_students[5]},(errInsert,result)=>{
		res.send(' successfully added');

	 })  
	 /* create collection course with fields*/

	 db.collection2.insert({id:sample_courses[0],longTitle:sample_courses[1],shortTitle: sample_courses[2],
		Professor:sample_courses[3],Type:sample_courses[4],Credits:sample_courses[5],
		Language:sample_courses[6],registeredStudent:sample_courses[7]},(errInsert,result)=>{
		res.send(' successfully added');
...
app.get('/register/:matriculationNumber/:shortTitle', (req, res) => {
	const client = req.app.locals.db;
	console.log("Read endpoint called with query");

	var db = client.db(database);
	var col = db.collection('students');
	matriculationNumber = (req.params._id)
	courseShortTitle = (req.params.short_title)

	col.update({_id:matriculationNumber},{$set: {courseShortTitle:courseShortTitle}},(err,result)=>{
		res.send('grade successfully updated')
	});
});
